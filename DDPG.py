...
class Actor(object):
    def __init__(self, sess, action_dim,
                 action_bound, learning_rate, t_replace_iter):
        self.sess = sess
    ...
    def _build_net(self, s, scope, trainable):
        with tf.variable_scope(scope):
            ...
            net = tf.layers.dense(s, 512, activation=tf.nn.relu,
            ...
            with tf.variable_scope('a'):
                actions = tf.layers.dense(net, self.a_dim,
    ...
    def add_grad_to_graph(self, a_grads):
        with tf.variable_scope('policy_grads'):
            self.policy_grads = tf.gradients(ys=self.a,
                                             xs=self.e_params, grad_ys=a_grads)

        with tf.variable_scope('A_train'):
            opt = tf.train.RMSPropOptimizer(-self.lr)
            self.train_op = \
                opt.apply_gradients(zip(self.policy_grads, self.e_params))


class Critic(object):
    def __init__(self, sess, state_dim,
                 action_dim, learning_rate, gamma, t_replace_iter, a, a_):
        self.sess = sess
        ...
        with tf.variable_scope('TD_error'):
            self.loss = \
                tf.reduce_mean(tf.squared_difference(self.target_q, self.q))
            tf.summary.scalar('TD_error', self.loss)
    ...
    def learn(self, s, a, r, s_):
        ...
        result = self.sess.run(merged,
                               feed_dict={S: s, self.a: a, R: r, S_: s_})
        writer.add_summary(result, self.t_replace_counter)
class Memory (object):
...
critic = Critic(sess, STATE_DIM, ACTION_DIM,
                LR_C, GAMMA, REPLACE_ITER_C, actor.a, actor.a_)
...
if LOAD:
    saver.restore(sess,'continuous/DDPG 4 .ckpt')
else:
...
def train():
    ...
    for ep in range(MAX_EPISODES):
        ...
        for t in range(MAX_EP_STEPS):
            ...
            for n in range(carn):
                ...
                # Added exploration noise
                    a = actor.choose_action(s[n])
                    a = np.clip(np.random.normal(a, var), *ACTION_BOUND)
                    s_[n], r[n], done[n] = env.step(a,n)

                    M.store_transition(s[n], a, r[n], s_[n])
            cgamar1 += r[n]
            if M.pointer > MEMORY_CAPACITY:
                var = max([var*.9998, VAR_MIN])
            ...
            if not False in done:
                dones = True
            if dones or t == MAX_EP_STEPS - 1:
                time2 = time.time()
                print(time2-time1, cgamar1, ep_step,)
        ...
        if (ep+1) % 250 == 0:
            ...
            ckpt_path = os.path.join(path, 'DDPG% i .ckpt' % int(i))
